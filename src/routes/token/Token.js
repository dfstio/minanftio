                        ...
                        {type === "video" ? (
                            <ReactPlayer
                                url={url == "https://ipfs.io/ipfs/QmREhQ5U9v68xPnTZaHusGHxmzCHBcLYLNQFyA2pJTWMLG"?
                                		`https://res.cloudinary.com/minanft/video/sp_auto/nft/@minanft/eiobsdaj2bjicxudkwzr` :
                                		url
                                }
                                controls={true}
                                //light={true}
        ...
        async function loadMedia() {
            if (firstRun) {
                ...
                if (item.markdown !== undefined)
                    setDescriptionMarkdown(item.markdown);
                setImage(
...
*/
        ...
        if (address !== undefined && address !== "") {
            ...
            if (result.publicKey !== "" && result.hash !== "") {
                ...
                while (newKey === "" && i < 20) {
                    ...
                    i++;
                    if (DEBUG)
                        console.log("Register - public key", i, ":", newKey);
                }
    ...
    const fetchUnlockable = async (newMedia, initial_count, count) => {
        ...
        let media2 = newMedia;
        if (DEBUG)
            console.log(`fetchUnlockable:`, initial_count, count, newMedia);
    ...
    const addUnlockable = async (media1, count) => {
        ...
        if (DEBUG)
            console.log(`addUnlockable media ${count}:`, newMedia, newAudio);
    ...
    const loadUnlockable = async (again = false) => {
        ...
        try {
            ...
            if (encryptedKey !== "") {
                ...
                const password = await metamaskDecrypt(
                    unlockableJSON.key,
                    address,
                );
                ...
                if (DEBUG)
                    console.log("View - Decrypted data: ", decryptedData);

                setCounter(counter + 1);
                await addUnlockable(
                    decryptedData.media,
                    decryptedData.media_count,
                );
                message.success({
                ...
                if (DEBUG)
                    console.log(`loadUnlockable media:`, media, "audio", audio);
            } else {
                message.error({
    ...
    async function showUnlockableContent() {
        ...
        if (address !== undefined && address !== "") {
            if (
                ...
                if (DEBUG)
                    console.log(
                        "showUnlockableContent wrong public key",
                        publicKey,
                    );
    ...
    function onSelect(id) {
        if (DEBUG)
            console.log("onSelect current", currentMedia, "selected", id);
    ...
    return (
      ...
      */}
                                    ...
                                    {small === false &&
                                    preview === false &&
                                    item.uri.contains_unlockable_content ===
                                        true &&
