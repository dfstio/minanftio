        ...
        if (signer && address !== "") {
            ...
            if (
                ...
                if (balance < MINIMUM_BALANCE) {
                    ...
                    const virtuosoInterface = new ethers.utils.Interface(
                        VirtuosoNFTJSON,
                    );
                    const data = virtuosoInterface.encodeFunctionData(
                        name,
                        args,
                    );
                ...
                log.error(
                    `wrong chain or address calling ${name} from ${address}`,
                );
        } else
...
export async function virtuosoRegisterPublicKey(address) {
    ...
    try {
        ...
        if (publicKey !== "") {
            ...
            let tx = await virtuosoFunction(address, "setPublicKey", [
                publicKey,
            ]);
            result.hash = tx.hash;
        }
