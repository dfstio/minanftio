...
var meta = {
    ...
    type: "functions",
    startTime: Date.now(),
    startFormattedTime: Date.now().toISOString().replace(/T/, " "),
    updated: true,
};

function initMeta() {
    meta = { id: uuidv4(), type: "functions", startTime: Date.now(), startFormattedTime : Date.now().toISOString().replace(/T/, " ") };
}

const myFormat = printf(({ level, message, winstonModule, wf, timestamp }) => {
    return `${timestamp} ${level} [${winstonModule} ${
        wf === undefined ? "" : ":" + wf
    }]: ${message}`;
});
...
async function flush() {
    await new Promise((resolve) => {
        transportInfo[1].kthxbye(resolve);
    });
    await new Promise((resolve) => {
        transportDebug[1].kthxbye(resolve);
    });
}
